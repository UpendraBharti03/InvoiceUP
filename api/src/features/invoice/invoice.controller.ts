...
import { createInvoice, getInvoiceDetails } from "@src/features/invoice/invoice.service";
...
const createInvoiceHandler = async (req: any, res: Response) => {
    ...
    try {
        ...
        const customer = await getCustomerDetails({ _id: reqBody?.companyId });
...
//     if (reqBody?.customerId) {
//         const customerObj = await getCustomerDetails({ _id: reqBody?.companyId });
